...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    rez = label * (feature_vector.dot(theta) + theta_0)
    hinge_loss = 0 if rez >= 1 else 1 - rez
    return hinge_loss
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    # rez = (labels * (feature_matrix.dot(theta) + theta_0)).mean()
    # loss = 0 if rez >= 1 else 1 - rez
    # return loss
    loss = 0
    for i, row in enumerate(feature_matrix):
        loss += hinge_loss_single(row, labels[i], theta, theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    theta, av_theta = np.zeros(feature_matrix.shape[1], ), \
                      np.zeros(feature_matrix.shape[1], )
    theta_0, av_theta_0 = 0, 0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            theta, theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], theta, theta_0)
            av_theta += theta
    ...
    return av_theta / n, av_theta_0 / n
...
def pegasos_single_step_update(
    ...
    completed.
    """
    if label * (feature_vector.dot(current_theta) + current_theta_0) <= 1:
        theta = (1 - eta * L) * current_theta + eta * label * feature_vector
        theta_0 = current_theta_0 + eta * label
        return theta, theta_0
    else:
        # no need in updating current_theta_0
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    cl = np.array([1 if x > 0 else -1 for x in feature_matrix.dot(theta) + theta_0])
    return cl
...
def bag_of_words(texts):
    ...
    #     stop_words = [word[:-1] for word in file.readlines()]
    stop_words = []
    for text in texts:
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                ...
                # feature_matrix[i, dictionary[word]] += 1    # for count features
